...
public abstract class PropertiesLoaderUtils {
    ...
    private static final Map<String, Properties> cache = Collections.synchronizedMap(new WeakHashMap<String, Properties>());
    ...
    public static Properties loadProperties(String resourceName, ClassLoader classLoader) throws IOException {
        ...
        if (props == null) {
            synchronized (cache) {
                props = cache.get(resourceName);
                if (props == null) {
                    props = new Properties();
                    loadIntoProperties(props, resourceName, classLoader);
                    cache.put(resourceName, props);
                }
            }
        }
        return props;
